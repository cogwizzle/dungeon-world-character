...
<link rel="stylesheet" href="tailwind.css" />\r
`;var j=(n,a)=>{let e=document.createElement("template"),t="bg-transparent border-b-2 border-b-black outline-none w-full";return e.innerHTML=`<input type="text" value="${n}" id="item-${a}" class="${t}" />`,e.content.firstChild},L=class extends HTMLElement{constructor(){super();s(this,"_value",[]);s(this,"onUpdate",async(e,t)=>{let o=e.target.value;if(this._value[t]=o,o===""){let i=this._value;this._value=this._value.filter((c,d)=>d!==t),this.renderList(i,this._value)}this.dispatchEvent(new CustomEvent("dw-infinite-list-change",{detail:this._value}))});s(this,"onAdd",e=>{let t=this._value.length;this._value=[...this._value,e],this.shadowRoot.querySelector("#new").value="";let o=j(e,t);o.setAttribute("value",e),this.shadowRoot.querySelector("#list").insertBefore(o,this.shadowRoot.querySelector("#new")),o.setSelectionRange(o.value.length,o.value.length),o.addEventListener("keyup",i=>{this.onUpdate(i,t)}),o.focus(),this.dispatchEvent(new CustomEvent("dw-infinite-list-change",{detail:this._value}))});this.attachShadow({mode:"open"})}get value(){return this._value}set value(e){this.renderList(this._value,e),this._value=e}diff(e,t){let[o,i]=t.reduce(([d,h],u,p)=>(p<e.length&&e[p]!==u&&h.push(p),p>=e.length&&d.push(u),[d,h]),[[],[]]);return{removed:e.reduce((d,h,u)=>(t[u]||d.push(u),d),[]),added:o,modified:i}}renderList(e=[],t=[]){let{removed:o,added:i,modified:c}=this.diff(e,t),d=this.shadowRoot.querySelector("#list");o.forEach(h=>{let u=this.shadowRoot.querySelector(`#item-${h}`);u&&(u.removeEventListener("keyup",p=>this.onUpdate(p,h)),u.remove())}),c.forEach(h=>{let u=this.shadowRoot.querySelector(`#item-${h}`);u&&(u.value=t[h])}),i.forEach((h,u)=>{let p=j(h,e.length+u);p.addEventListener("keyup",V=>this.onUpdate(V,u)),d.insertBefore(p,this.shadowRoot.querySelector("#new"))})}connectedCallback(){this.render(),this.shadowRoot.querySelector("#new").addEventListener("keyup",e=>this.onAdd(e.target.value))}disconnectedCallback(){this.shadowRoot.querySelector("#new").removeEventListener("keyup",e=>this.onAdd(e.target.value)),this._value.forEach((e,t)=>{document.querySelector(`#item-${t}`).removeEventListener("keyup",i=>this.onUpdate(i,t))})}render(){this.shadowRoot.innerHTML=I,this.renderList([],this._value)}};customElements.define("dw-infinite-list",L);var S=class extends HTMLElement{constructor(){super();s(this,"hydrate",e=>{this.querySelector("#bonds-list").value=e.bonds})}onUpdate(e){r.bonds=e.detail}connectedCallback(){this.render(),r.subscribe(this.hydrate),this.querySelector("#bonds-list").addEventListener("dw-infinite-list-change",this.onUpdate)}disconnectedCallback(){r.unsubscribe(this.hydrate),this.querySelector("#bonds-list").removeEventListener("dw-infinite-list-change",this.onUpdate)}render(){this.innerHTML=N}};customElements.define("dw-character-bonds",S);var B=`<dw-vertical-selection id="race" title="race"></dw-vertical-selection>\r
`;var m=(n,a)=>({name:n,description:a}),M=class extends HTMLElement{constructor(){super();s(this,"_characterClass");s(this,"onChange",e=>{r.race=e.detail.value});s(this,"hydrate",e=>{this._characterClass!==e.characterClass&&(this._characterClass=e.characterClass,this.querySelector("#race").setAttribute("value",e.race),this.updateOptions())})}get options(){switch(this._characterClass){case l.Fighter:return[m("dwarf","When you share a drink with someone, you may parley with them using CON instead of CHA."),m("elf","Choose one weapon-you can always treat weapons of that type as if they had the precise tag."),m("halfling","Choose one weapon-you can always treat weapons of that type as if they had the precise tag."),m("human","Once per battle you may reroll a single damage roll (yours or someone else's")];case l.Ranger:return[m("elf","When you Undertake A Perilous Journey through wilderness whatever job you take you succeed as if you rolled a 10+."),m("human","When you Make Camp in a dungeon or city, you don't need to consume a ration.")];case l.Bard:return[m("elf","When you enter an important location (your call) you can ask the GM for one fact from the history of that location."),m("human","When you first enter a civilized settlement someone who respects the custom of hospitality to minstrels will take you in as their guest.")];case l.Druid:return[m("elf","The sap of the elder trees flows within you. In addition to any other attunements, the Great Forest is always considered your land."),m("human","As your people learned to bind animals to field and farm, so too are you bound to them. You may always take the shape of any domesticated animal, in addition to your normal options."),m("halfling","You sing the healing songs of spring and brook. When you make camp, you and your allies heal +1d6.")];case l.Paladin:return[m("human","When you pray for guidance, even for a moment and ask \u201Cwhat here is evil?\u201D the GM will tell you, honestly.")];default:return[]}}connectedCallback(){this.render(),r.subscribe(this.hydrate),this.querySelector("#race").addEventListener("dw-change",this.onChange)}disconnectedCallback(){r.unsubscribe(this.hydrate),this.querySelector("#race").addEventListener("dw-change",this.onChange)}updateOptions(){this.querySelector("#race").setAttribute("options",JSON.stringify(this.options))}render(){this.innerHTML=B,this.updateOptions()}};customElements.define("dw-character-race",M);var G=`<style>\r
  .coin {\r
    top: -0.4rem;\r
  }\r
</style>\r
